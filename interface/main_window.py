...
class MainContent:
    ...
    def _build_layout(self):
        ...
        self._build_sidebar()
        self.content_frame = ctk.CTkFrame(self.container, fg_color="#F4F6FB")
        self.content_frame.pack(side="left", fill="both", expand=True)

    def _build_sidebar(self):
        sidebar = ctk.CTkFrame(self.container, width=240, fg_color="#0B2A4A", corner_radius=0)
        ...
        ctk.CTkLabel(logo_frame, text="HAN", font=("Segoe UI", 20, "bold"),
                     text_color="white").grid(row=0, column=1, sticky="w")
        ctk.CTkLabel(logo_frame, text="Hospital Agostinho Neto", font=("Segoe UI", 13),
                     text_color="#D6E4F0").grid(row=1, column=1, sticky="w")
        ...
        for nome, icon_path in MENUS:
            ...
            btn = ctk.CTkButton(
                sidebar, text=nome, image=img, compound="left",
        ...
        try:
            sair_img = ctk.CTkImage(Image.open("assets/sair.png"), size=(20, 20))
            self._sair_ref = sair_img
        except Exception:
            sair_img = None

        ctk.CTkButton(
            sidebar, text="Terminar Sessão", image=sair_img, compound="left",
            fg_color="transparent", text_color="#FF6B6B",
            hover_color="#2A3F5F", anchor="w", height=45,
            command=self.terminar_sessao,
        ).pack(side="bottom", fill="x", padx=15, pady=20)

        ctk.CTkFrame(sidebar, height=1, fg_color="#35506E").pack(
            side="bottom", fill="x", padx=15, pady=(0, 10))
    ...
    def load_screen(self, menu_name):
        ...
        if menu_name == "Painel Principal":
            from interface.dashboard import DashboardContent
